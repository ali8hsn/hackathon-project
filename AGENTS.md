...
- Project rebrand: replace every "Sentinel" reference (code, identifiers, routes, branding, comments, docs) with "Haashir" — except the MongoDB database name.
...
- MongoDB Atlas database name MUST remain `sentinel` to keep existing live incident data accessible — do not rename despite the Haashir rebrand. Document this exception in `.env.example`, `setup-db.ts`, `DEPLOY_NOTES.md`, etc.
...
- Backfill script `scripts/backfill-severity.mjs` rescores existing incidents via Gemini; run with `MONGODB_URI`, `MONGODB_DB_NAME=sentinel`, and `GEMINI_API_KEY` env vars after schema-affecting changes.
